...
def add_bias_1(cur, scope):
    with tf.compat.v1.variable_scope(scope):
        ...
        offset = tf.Variable(
            tf.zeros_initializer, name="offset", shape=[1, 1, size[-1]]
        )
        return cur + offset
...
def conv_linear(
    ...
    with tf.compat.v1.variable_scope(prefix):
        ...
        if kernel_width == 1:
            ...
            filter = tf.Variable(
                name="CvK", shape=[nin, nout], initial_value=initializer
            )

            res = tf.matmul(
                tf.reshape(input, [inp_shape[0] * inp_shape[1], nin]), filter
            )
            res = tf.reshape(res, [inp_shape[0], inp_shape[1], nout])
        else:
            # filter = tf.get_variable("CvK", [kernel_width, nin, nout], initializer=initializer)
            filter = tf.Variable(
                name="CvK", shape=[kernel_width, nin, nout], initial_value=initializer
            )
            res = tf.nn.conv1d(
                input=input, filters=filter, stride=stride, padding="SAME"
            )
        ...
        if add_bias:
            ...
            bias_term = tf.Variable(
                name="CvB",
                shape=[nout],
                initial_value=tf.compat.v1.random_uniform_initializer(
                    bias_start, bias_start + 0.01
                ),
            )
            res = res + bias_term
...
def switch_layer(mem_shuffled, kernel_width, prefix):
    ...
    residual_scale = tf.sigmoid(
        tf.Variable(
            name=prefix + "/residual",
            shape=[num_units * 2],
            initial_value=tf.constant_initializer(
                inv_sigmoid(residual_weight) / lr_adjust
            ),
        )
        * lr_adjust
    )
    residual_list.append(
        tf.reshape(tf.clip_by_value(residual_scale, 0.0, 1.0), [1, num_units * 2, 1])
    )
...
def shuffle_exchange_network_heavy_sharing(
    cur,
    name,
    kernel_width=1,
    n_blocks=1,
    tied_inner_weights=True,
    ...
    with tf.compat.v1.variable_scope(
        name + "_recursive", reuse=tf.compat.v1.AUTO_REUSE
    ):
        for k in range(n_blocks):
            ...
            for i in range(n_bits - 1):
                outstack.append(cur)
                layer_name = "forward"
                if not tied_outer_weights:
                    layer_name = str(k) + "_" + layer_name
                if not tied_inner_weights:
                    layer_name += "_" + str(i)
...
def shuffle_exchange_network(
    ...
    with tf.compat.v1.variable_scope(
        name + "/shuffle_exchange", reuse=tf.compat.v1.AUTO_REUSE
    ):
        outstack = []
        stack = []
        ...
        def switch_and_shuffle(cur, do_ror, layer_name, block_index, layer_index):
            ...
            if not tied_outer_weights or prev is None:
                layer_name = str(block_index) + "_" + layer_name
            if not tied_inner_weights:
